...
namespace Negocio
{
    public class ArticuloNegocio
    {
        ...
        public List<Articulo> listar(string seleccion)
        {
            List<Articulo> lista = new List<Articulo>();
            establecerConexion();
            try
            {
                datos.setearConsulta("Select a.Id, a.Codigo, a.Nombre, a.descripcion, a.IdMarca, a.IdCategoria, a.ImagenUrl, a.precio, c.Descripcion Categoria, m.Descripcion Marca from Articulos a inner join Categorias C on a.IdCategoria = c.Id inner join Marcas M on a.IdMarca = m.Id" + ArmarOrden(seleccion));
                datos.ejecutarLector();
                while (datos.Lector.Read())
                {
                    Articulo aux = new Articulo();
